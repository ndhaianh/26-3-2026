...
class Particle {
    ...
    draw() {
        ...
        const rgb = document.documentElement.getAttribute('data-theme') === 'light' ? '0, 180, 216' : '0, 243, 255';
        ctx.fillStyle = `rgba(${rgb}, ${this.opacity || 1})`;
        ctx.fill();
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const defaultMessages = [
        {
            sender: 'Hệ thống A3',
            text: 'Chào mừng đến Trạm Phát Tín Hiệu Thanh Xuân của 12A3! 🌸 Gửi lại lời nhắn cho những tháng ngày sắp xa nhau nhen.',
...
// ==================== HEIC IMAGE SUPPORT ====================
// Tự động convert ảnh .heic/.HEIC sang JPEG để hiển thị trên trình duyệt
(function () {
    function isHEIC(src) {
        return src && /\.heic$/i.test(src);
    }

    async function convertHEICImages() {
        if (typeof heicTo === 'undefined') return; // heic-to library not loaded
        ...
        for (const img of images) {
            if (isHEIC(img.src)) {
                try {
                    ...
                    console.warn('HEIC conversion failed for:', img.src, err);
                }
            }
        }
    }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => setTimeout(convertHEICImages, 500));
    } else {
        setTimeout(convertHEICImages, 500);
    }
})();
...
// ==================== LIGHTBOX GALLERY ====================
document.addEventListener('DOMContentLoaded', () => {
    // Tạo cấu trúc HTML cho lightbox và thêm vào body
    ...
    function closeLightbox() {
        ...
        setTimeout(() => {
            lightboxMedia.innerHTML = ''; // Xóa nội dung để video ngừng phát
        }, 300);
    }

    closeBtn.addEventListener('click', closeLightbox);
    
    // Đóng khi click ra ngoài ảnh/video
    ...
    // Bắt sự kiện click vào các item trong gallery (Dùng Event Delegation)
    document.addEventListener('click', (e) => {
        ...
        if (item) {
            ...
            lightboxMedia.innerHTML = ''; // Làm sạch trước khi thêm mới
            ...
            if (img) {
                ...
                clone.src = video.getAttribute('src') || video.currentSrc;
                clone.controls = true;
                clone.autoplay = true;
                lightboxMedia.appendChild(clone);
            } else {
                return; // Nếu là placeholder (không có media thật) thì bỏ qua
            }
            
            lightboxCaption.innerText = title.replace('🎬 ', ''); // Bỏ icon video nếu có
